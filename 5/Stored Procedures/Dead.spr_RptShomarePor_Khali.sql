...
SET ANSI_NULLS ON
GO
CREATE proc [Dead].[spr_RptShomarePor_Khali](@fieldName nvarchar(50),@vadiSalam varchar(10),@organ varchar(10),@GheteId varchar(20),@radifid nvarchar(20))
as
--declare @fieldName nvarchar(50)=N'khali',@vadiSalam varchar(10)='1',@organ varchar(10)=1,@GheteId varchar(20)='1',@radifid nvarchar(20)='1'
...
;with tabaghe as 
	...
	if (@fieldname='Khali')
	begin
	set @query=@query+ ' select fldGheteId,RadifId, fldShomare,fldNameRadif,fldNameGhete as NameGhete, ShomareId,fldNamevadiSalam,''''as Name_Family,''''Meli_Moshakhase,''''As FatherName,''''Sh_Sh,'''' as TarikhFot,''''As Tabaghe 
	,''''TabagheKhali
	from tabaghe
	 outer apply (select 1 as id ,fldshomareid  from dead.tblRequestAmanat

				 left join dead.tblKartabl_Request on fldRequestId=tblRequestAmanat.fldid
				 where  fldShomareId=tabaghe.ShomareId and fldIsEbtal<>1 
	...
	where  not exists (select * from dead.tblGhabreAmanat inner join dead.tblshomare on fldShomareId=tblShomare.fldid
					 where fldShomareId=tabaghe.ShomareId)
	...
	if (@radifid<>'0')
	 set @query=@query+ ' and RadifId='+@radifid

	 set @query=@query+ ' group by fldShomare,fldNameRadif,fldNameGhete,ShomareId,fldNamevadiSalam, tabaghe.fldGheteId,RadifId
						order by fldNameGhete' 
						
	end
	if (@fieldname='Por')
	begin
	set @query=@query+ 'select q. fldGheteId,RadifId,fldShomare,fldNameRadif, NameGhete,ShomareId,fldNamevadiSalam,
	Name_Family,Meli_Moshakhase, FatherName,Sh_Sh, TarikhFot
	,Tabaghe,TabaghatKhali  from 
   ...
   from tabaghe
	...
	cross apply(select (fldTedadTabaghat)sumkol from  dead.tblshomare 
					where tblshomare.fldid=ShomareId
						)sumradif

	outer apply(select isnull(( select count (tblGhabreAmanat.fldid)
	from dead .tblGhabreAmanat 
	where fldShomareId=q.ShomareId)	,0)+		
	isnull((select count( tedad) tedad  from ( select (tblRequestAmanat.fldid) tedad
						 from dead .tblRequestAmanat 
						left join dead.tblKartabl_Request on fldRequestId=tblRequestAmanat.fldid
						where  fldShomareId=q.ShomareId and fldIsEbtal <>1 and (tblKartabl_Request.fldid is null or (tblKartabl_Request.fldid is not null and fldEtmamCharkhe<>1 ))
						group by tblRequestAmanat.fldid)t)
	,0)tedadporshode)tedadama
	...
	if (@GheteId<>'0')
	 ...
	 set @query=@query+ '
			 group by  q. fldGheteId,RadifId,fldShomare,fldNameRadif, NameGhete,ShomareId,fldNamevadiSalam,
							Name_Family,Meli_Moshakhase, FatherName,Sh_Sh, TarikhFot
	...
	outer apply (select count(*)tedad from dead.tblGhabreAmanat
	 ...
	 outer apply (select count(tedad)tedad from (select (tblRequestAmanat.fldid)tedad from dead.tblRequestAmanat
				 left join dead.tblKartabl_Request on fldRequestId=tblRequestAmanat.fldid
				 where  fldShomareId=ShomareId and fldIsEbtal<>1 
				 and (tblKartabl_Request.fldid is null or (tblKartabl_Request.fldid is not null and fldEtmamCharkhe<>1) )
				group by tblRequestAmanat.fldid)t )requestpor
	)t
	where tedadpor<fldTedadTabaghat  and tedadpor>0
		'
		if (@GheteId<>'0')
	 set @query=@query+ ' and fldGheteId='+@GheteId
	 ...
	 set @query=@query+ ' and RadifId='+@radifid 
		...
		set @query=@query+'
		 group by   t.fldGheteId, RadifId,fldShomare,fldNameRadif, NameGhete, ShomareId,fldNamevadiSalam
					,Name_Family,	Meli_Moshakhase, FatherName,Sh_Sh, TarikhFot,Tabaghe,TabagheKhali
						, fldTedadTabaghat,tedadpor
