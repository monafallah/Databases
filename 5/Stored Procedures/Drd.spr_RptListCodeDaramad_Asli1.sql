...
SET ANSI_NULLS ON
GO
create PROC [Drd].[spr_RptListCodeDaramad_Asli1](@AzTarikh NVARCHAR(10),@TaTarikh NVARCHAR(10),@ShomareHesabId INT,@CodeDaramadId INT,@OrganId INT,@fieldname NVARCHAR(50),@DateType tinyint)
AS
...
BEGIN
		...
		FROM            Drd.tblRequestTaghsit_Takhfif INNER JOIN
			 ...
			 begin
				...
				ISNULL((SELECT d.fldStrhid FROM Drd.tblCodhayeDaramd AS d WHERE c.flddaramadId.GetAncestor(1)=d.fldDaramadId AND d.fldLevel<>0),'') AS P_hid,
				ISNULL((SELECT fldDaramadCode FROM Drd.tblCodhayeDaramd AS d WHERE c.flddaramadId.GetAncestor(1)=d.fldDaramadId AND d.fldLevel<>0),'') AS P_DaramadCode,
				/*ISNULL((SELECT d.fldId FROM Drd.tblCodhayeDaramd AS d WHERE c.flddaramadId.GetAncestor(1)=d.fldDaramadId ),'')*/c.fldid AS P_IdCode,
				ISNULL((SELECT fldDaramadTitle FROM Drd.tblCodhayeDaramd AS d WHERE c.flddaramadId.GetAncestor(1)=d.fldDaramadId AND d.fldLevel<>0),'') AS P_DaramadTitle,
				ISNULL((SELECT fldLevel FROM Drd.tblCodhayeDaramd AS d WHERE c.flddaramadId.GetAncestor(1)=d.fldDaramadId AND d.fldLevel<>0),'') AS P_Level
				,sum(mablaghcodeDaramad) over (partition by tem.Idcode,tem.ShomareHId)/*ISNULL((SELECT SUM(mablaghcodeDaramad) FROM @temp WHERE Daramadcode=C.fldDaramadCode) ,ISNULL((SELECT SUM(mablaghcodeDaramad) FROM @temp AS t WHERE t.hid.ToString() LIKE c.fldStrhid+'%'),0))*/ AS Mablagh
				,fldLevel,fldDaramadTitle,fldDaramadCode				
			...
			select distinct  r.* from (	select  sum(Mablagh) Mablagh,P_DaramadTitle,t.fldDaramadCode,fldDaramadId ,fldLevel,fldStrhid
				from (  select p2.fldDaramadId, p2.fldDaramadCode,p2.fldLevel,p2.fldStrhid,/*case when c.fldDaramadTitle=N'پیش فرض' then d.P_DaramadTitle else  c.fldDaramadTitle end*/p2.fldDaramadTitle  as P_DaramadTitle/*P_DaramadTitle*/,(Mablagh)Mablagh,tedad ,fldTarikh,fldShomareHesab,fldShomareHesabId 
				from @table as d inner join drd.tblCodhayeDaramd c on c.fldid=d.P_Idcode
				left join drd.tblCodhayeDaramd as p2   on   c.fldDaramadId.IsDescendantOf(p2.fldDaramadId)=1
				
			--	outer apply (select (select p2.fldDaramadTitle+'/' from drd.tblCodhayeDaramd as p2 where c.fldDaramadId.IsDescendantOf(p2.fldDaramadId)=1 /*and p.fldHeaderCodId=p2.fldHeaderCodId */for xml path('')) as fldParentTitle )t
			where p2.fldLevel<>0
				 group by P_DaramdCode,p2.fldDaramadTitle,tedad,fldTarikh,p2.fldStrhid,fldShomareHesab,fldShomareHesabId,p2.fldDaramadCode,p2.fldDaramadId,Mablagh,p2.fldLevel--,P_Idcode,d.P_DaramadTitle
				)t
				group by P_DaramadTitle,t.fldDaramadCode,fldDaramadId,fldLevel,fldStrhid
				)r
				left join drd.tblCodhayeDaramd c3 on r.fldDaramadId= c3.fldDaramadId.GetAncestor(1) 
			where  c3.fldDaramadId is not null --and r.fldLevel>1*/
				order by r.fldDaramadCode,r.fldLevel,fldStrhid
