...
SET ANSI_NULLS ON
GO
CREATE PROC [Drd].[spr_FishSaderShodeForXmlOut]
...
,
(SELECT   (CASE WHEN fldHoghoghiId IS NOT NULL  THEN (CASE WHEN EXISTS(SELECT * FROM Com.tblAshkhaseHoghoghi WHERE  tblAshkhaseHoghoghi.fldId =fldHoghoghiId AND fldSayer=1) THEN N'2'
WHEN EXISTS(SELECT * FROM Com.tblAshkhaseHoghoghi WHERE  tblAshkhaseHoghoghi.fldId =fldHoghoghiId AND fldSayer=2) THEN N'0' end  )
 WHEN fldHaghighiId IS NOT NULL THEN N'1' end)  
FROM         Drd.tblElamAvarez INNER JOIN
