...
SET ANSI_NULLS ON
GO
CREATE proc [Pay].[spr_Pay_RptFishHoghoghi_Motalebat_Details]
@fldPersonalId INT,
@NobatPardakht INT,
@AzYear INT,
@TaYear INT
...
where i.fldHesabTypeItemId= @dHesabType  and  fldPersonalId=@fldPersonalId and fldYear*100+fldMonth>=@AzYear AND fldYear*100+fldMonth<=@TaYear AND fldNobatPardakht=@NobatPardakht  
and fldItemsHoghughiId not in (67,68,76) and fldCalcType=@CalcType

union all
select  e.fldItemsHoghughiId,e.fldTitle+ a.fldNameMonasebat+'('+t.fldTypeName+')',i.fldMablagh,fldMashmolBime,fldMashmolMaliyat,fldPersonalId,fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht,fldYear,m.fldMonth,fldKarkard,p.fldHokmId,1 fldtype,1 as fldCalcType
,case when i.fldMaliyatMashmool= 1 then N'هست' when i.fldMaliyatMashmool= 0 then  N'نیست' else N'نامشخص' end as fldMaliyatMashmool
,case when i.fldBimeMashmool= 1 then N'هست' when i.fldBimeMashmool= 0 then N'نیست' else N'نامشخص' end as fldBimeMashmool
,isnull(h.fldMablagh,0) as fldMablaghHokm
from Pay.tblMohasebat as m
...
left join prs.tblHokm_Item as h on h.fldPersonalHokmId=p.fldHokmId and h.fldItems_EstekhdamId=i.fldItemEstekhdamId
where i.fldHesabTypeItemId= @dHesabType  and  fldPersonalId=@fldPersonalId and fldYear*100+m.fldMonth>=@AzYear AND fldYear*100+m.fldMonth<=@TaYear AND fldNobatPardakht=@NobatPardakht  
...
+')',i.fldMablagh,fldMashmolBime,fldMashmolMaliyat,m.fldPersonalId,fldYear as fldYearPardakht,fldMonth as fldMonthPardakht,fldYear,fldMonth,fldKarkard,p.fldHokmId,1 fldtype,1 as fldCalcType
,case when i.fldMaliyatMashmool= 1 then N'هست' when i.fldMaliyatMashmool= 0 then  N'نیست' else N'نامشخص' end as fldMaliyatMashmool
,case when i.fldBimeMashmool= 1 then N'هست' when i.fldBimeMashmool= 0 then N'نیست' else N'نامشخص' end as fldBimeMashmool
,isnull(h.fldMablagh,0) as fldMablaghHokm
from Pay.tblMohasebat as m
...
left join prs.tblHokm_Item as h on h.fldPersonalHokmId=p.fldHokmId and h.fldItems_EstekhdamId=i.fldItemEstekhdamId
where i.fldHesabTypeItemId= @dHesabType  and  m.fldPersonalId=@fldPersonalId and fldYear*100+fldMonth>=@AzYear AND fldYear*100+fldMonth<=@TaYear AND fldNobatPardakht=@NobatPardakht  
...
where  tblMohasebat.fldPersonalId=@fldPersonalId and fldYear*100+fldMonth>=@AzYear AND fldYear*100+fldMonth<=@TaYear AND fldNobatPardakht=@NobatPardakht  AND 
...
FROM         Pay.tblMoavaghat_Items INNER JOIN
					...
					left join prs.tblHokm_Item as h on h.fldPersonalHokmId=tblMoavaghat.fldHokmId and h.fldItems_EstekhdamId=tblMoavaghat_Items.fldItemEstekhdamId
					  cross apply(select mm.fldKarkard from pay.tblMohasebat as mm where mm.fldPersonalId=m.fldPersonalId and mm.fldYear=tblMoavaghat.fldYear and mm.fldMonth=tblMoavaghat.fldMonth  and mm.fldCalcType=@CalcType)mm
                      WHERE   fldHesabTypeItemId= @dHesabType  and   fldPersonalId=@fldPersonalId and m.fldYear*100+m.fldMonth>=@AzYear AND m.fldYear*100+m.fldMonth<=@TaYear AND fldNobatPardakht=@NobatPardakht   
...
CASE WHEN fldNesbatShakhs=1 THEN N'فرزند' WHEN fldNesbatShakhs=2 THEN N'همسر' WHEN fldNesbatShakhs=3 THEN N'پدر' WHEN fldNesbatShakhs=4 THEN N'مادر' END
+')'+N'_معوقه',  (Pay.tblMoavaghat_Items.fldMablagh) AS Expr1,tblMoavaghat.fldMashmolBime,tblMoavaghat.fldMashmolMaliyat,m.fldPersonalId,m.fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht,tblMoavaghat.fldyear,tblMoavaghat.fldmonth,mm.fldKarkard,tblMoavaghat.fldHokmId,7 fldtype,2 as fldCalcType
,case when tblMoavaghat_Items.fldMaliyatMashmool= 1 then N'هست' when tblMoavaghat_Items.fldMaliyatMashmool= 0 then  N'نیست' else N'نامشخص' end as fldMaliyatMashmool
,case when tblMoavaghat_Items.fldBimeMashmool= 1 then N'هست' when tblMoavaghat_Items.fldBimeMashmool= 0 then N'نیست' else N'نامشخص' end as fldBimeMashmool
,isnull(h.fldMablagh,0) as fldMablaghHokm
FROM         Pay.tblMoavaghat_Items INNER JOIN
					...
					left join prs.tblHokm_Item as h on h.fldPersonalHokmId=tblMoavaghat.fldHokmId and h.fldItems_EstekhdamId=tblMoavaghat_Items.fldItemEstekhdamId
					  cross apply(select mm.fldKarkard from pay.tblMohasebat as mm where mm.fldPersonalId=m.fldPersonalId and mm.fldYear=tblMoavaghat.fldYear and mm.fldMonth=tblMoavaghat.fldMonth   and mm.fldCalcType=@CalcType)mm
                      WHERE   fldHesabTypeItemId= @dHesabType  and   m.fldPersonalId=@fldPersonalId and m.fldYear*100+m.fldMonth>=@AzYear AND m.fldYear*100+m.fldMonth<=@TaYear AND fldNobatPardakht=@NobatPardakht   
...
union all
		...
		cross apply(select mm.fldKarkard from pay.tblMohasebat as mm where mm.fldPersonalId=m.fldPersonalId and mm.fldYear=o.fldYear and mm.fldMonth=o.fldMonth   and mm.fldCalcType=@CalcType)mm
		WHERE  @dHesabType<>1 and  fldPersonalId=@fldPersonalId and m.fldYear*100+m.fldMonth>=@AzYear AND m.fldYear*100+m.fldMonth<=@TaYear AND fldNobatPardakht=@NobatPardakht   
					  and o.fldHaghDarmanKarfFarma>0   and fldCalcType=@CalcType
		union all
...
union all
SELECT  i.fldItemsHoghughiId,i.fldTitle+N'_معوقه متمم ماه قبل',  (o.fldMablagh) AS Expr1,m.fldMashmolBime,m.fldMashmolMaliyat,m.fldPersonalId,[Com].[fn_GetNextYear](m.fldYear,m.fldmonth) as fldYearPardakht,[Com].[fn_GetNextMonth](m.fldYear,m.fldmonth) as fldMonthPardakht,m.fldyear,m.fldmonth,mm.fldKarkard,p.fldHokmId,11 fldtype,3 as fldCalcType
...
FROM         Pay.tblMohasebat_ItemMotamam as o INNER JOIN
					...
					left join prs.tblHokm_Item as h on h.fldPersonalHokmId=p.fldHokmId and h.fldItems_EstekhdamId=o.fldItemEstekhdamId
					  ...
					  union all
SELECT  i.fldItemsHoghughiId,i.fldTitle+ a.fldNameMonasebat+'('+t.fldTypeName+')'+N'_معوقه متمم ماه قبل',  (o.fldMablagh) AS Expr1,m.fldMashmolBime,m.fldMashmolMaliyat,m.fldPersonalId,[Com].[fn_GetNextYear](m.fldYear,m.fldmonth) as fldYearPardakht,[Com].[fn_GetNextMonth](m.fldYear,m.fldmonth) as fldMonthPardakht,m.fldyear,m.fldmonth,mm.fldKarkard,p.fldHokmId,12 fldtype,3 as fldCalcType
,case when o.fldMaliyatMashmool= 1 then N'هست' when o.fldMaliyatMashmool= 0 then  N'نیست' else N'نامشخص' end as fldMaliyatMashmool
,case when o.fldBimeMashmool= 1 then N'هست' when o.fldBimeMashmool= 0 then N'نیست' else N'نامشخص' end as fldBimeMashmool
,isnull(h.fldMablagh,0) as fldMablaghHokm
FROM         Pay.tblMohasebat_ItemMotamam as o INNER JOIN
...
SELECT  i.fldItemsHoghughiId,i.fldTitle+'('+
CASE WHEN fldNesbatShakhs=1 THEN N'فرزند' WHEN fldNesbatShakhs=2 THEN N'همسر' WHEN fldNesbatShakhs=3 THEN N'پدر' WHEN fldNesbatShakhs=4 THEN N'مادر' END
+')'+N'_معوقه متمم ماه قبل',  (o.fldMablagh) AS Expr1,m.fldMashmolBime,m.fldMashmolMaliyat,m.fldPersonalId,[Com].[fn_GetNextYear](m.fldYear,m.fldmonth) as fldYearPardakht,[Com].[fn_GetNextMonth](m.fldYear,m.fldmonth) as  fldMonthPardakht,m.fldyear,m.fldmonth,mm.fldKarkard,p.fldHokmId,13 fldtype,3 as fldCalcType
,case when o.fldMaliyatMashmool= 1 then N'هست' when o.fldMaliyatMashmool= 0 then  N'نیست' else N'نامشخص' end as fldMaliyatMashmool
,case when o.fldBimeMashmool= 1 then N'هست' when o.fldBimeMashmool= 0 then N'نیست' else N'نامشخص' end as fldBimeMashmool
,isnull(h.fldMablagh,0) as fldMablaghHokm
FROM         Pay.tblMohasebat_ItemMotamam as o INNER JOIN
...
/*union all
		select 0 as fldItemsHoghughiId, N'حق درمان کارفرما_متمم' AS fldTitle,(fldMablagh),fldMashmolBime,fldMashmolMaliyat,fldPersonalId, fldYearPardakht, fldMonthPardakht,fldYear,fldMonth,fldKarkard,fldHokmId,14 fldtype,3 as fldCalcType
		,N'نیست' fldMaliyatMashmool,N'نیست' fldBimeMashmool,0 as fldMablaghHokm
		from(
		
		SELECT    (m.fldHaghDarmanKarfFarma) as fldMablagh,m.fldMashmolBime,m.fldMashmolMaliyat,fldPersonalId,m.fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht ,m.fldyear,m.fldmonth,p.fldHokmId,mm.fldKarkard
		FROM pay.tblMohasebat as m
		inner join Pay.tblMohasebat_ItemMotamam as o on m.fldId=o.fldMohasebatId  inner join
					  pay.tblMohasebat_PersonalInfo as p on p.fldMohasebatId = m.fldId
		cross apply(select mm.fldKarkard from pay.tblMohasebat as mm where mm.fldPersonalId=m.fldPersonalId and mm.fldYear=m.fldYear and mm.fldMonth=m.fldMonth  and mm.fldCalcType=2)mm
		WHERE  @dHesabType<>1 and  fldPersonalId=@fldPersonalId and m.fldYear*100+m.fldMonth>=com.fn_GetPreviousMonth(@AzYearp,@AzMonthP) AND  m.fldYear*100+m.fldMonth<=com.fn_GetPreviousMonth(@TaYearp,@TaMonthP) AND fldNobatPardakht=@NobatPardakht   
					  and m.fldHaghDarmanKarfFarma>0   and fldCalcType=@CalcType
		union all
		SELECT    (m.fldHaghDarmanDolat) as fldMablagh,m.fldMashmolBime,m.fldMashmolMaliyat,fldPersonalId,m.fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht,m.fldyear,m.fldmonth,p.fldHokmId,mm.fldKarkard 
		FROM pay.tblMohasebat as m
		inner join Pay.tblMohasebat_ItemMotamam as o on m.fldId=o.fldMohasebatId  inner join
					  pay.tblMohasebat_PersonalInfo as p on p.fldMohasebatId = m.fldId
		cross apply(select mm.fldKarkard from pay.tblMohasebat as mm where mm.fldPersonalId=m.fldPersonalId and mm.fldYear=m.fldYear and mm.fldMonth=m.fldMonth  and mm.fldCalcType=2)mm
...
union all
        SELECT     0 as fldItemsHoghughiId,N'پس انداز کارفرما_متمم' AS fldTitle, m.fldPasAndaz/2 ,m.fldMashmolBime,m.fldMashmolMaliyat,fldPersonalId,m.fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht,m.fldYear,m.fldMonth,mm.fldKarkard,p.fldHokmId,15 fldtype,3 as fldCalcType
	,N'نیست' fldMaliyatMashmool,N'نیست' fldBimeMashmool,0 as fldMablaghHokm
	FROM         Pay.tblMohasebat as m
         inner join Pay.tblMohasebat_ItemMotamam as o on m.fldId=o.fldMohasebatId  inner join
					  pay.tblMohasebat_PersonalInfo as p on p.fldMohasebatId = m.fldId
