...
SET ANSI_NULLS ON
GO
CREATE proc [Auto].[spr_CharkheMessage](@MessageId int,@ChPid int)
as
...
WHERE     fldAssignmentID IN (select fldId from tblAssignment where fldMessageId=@MessageId)
...
if(@ChPid is null)
	select  id,sender,null pid,AssId,_SID as C_SID, SenderName,'' fldTypeAss,'' fldNameStatus,'' fldLetterReadDate,'' fldTarikhErja,'' fldDesc
					  from @ch 
	...
	select id,sender, pid,AssId,_SID as C_SID ,SenderName,fldType fldTypeAss,statusname fldNameStatus, statusass.LetterReadDate fldLetterReadDate,statusass.fldTarikhErja fldTarikhErja, fldDesc
							 from @ch
						  cross apply (SELECT    tblEmployee.fldName COLLATE Latin1_General_CS_AS + ' ' + tblEmployee.fldFamily COLLATE Latin1_General_CS_AS + '(' + tblOrganizationalPostsEjraee.fldTitle + ')'  AS SenderName
					 from tblCommision INNER JOIN
						  com.tblAshkhas ON tblCommision.fldAshkhasID = tblAshkhas.fldID INNER JOIN
