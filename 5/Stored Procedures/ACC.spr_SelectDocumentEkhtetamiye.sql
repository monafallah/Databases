...
SET ANSI_NULLS ON
GO
CREATE proc [ACC].[spr_SelectDocumentEkhtetamiye]
@FiscalYearId int,
@Type tinyint
as 
BEGIN TRAN
--declare @FiscalYearId int=6,@Type tinyint=1
declare @year smallint=0,@headerId int=0
if(@Type=1)
begin
	select @year=fldYear+1 from acc.tblFiscalYear where fldid=@FiscalYearId
	select @headerId=fldId  from acc.tblCoding_Header where fldYear=@year
end

select  fldCodingId
,case when @Type=1 then (select c.fldTitle   from acc.tblCoding_Details as c where c.fldid=fldCodingId) else fldTitle end fldTitle
,N'' as fldDescription
,case when mande>0 then mande else 0 end as fldBestankar   
,case when mande<0 then abs(mande) else 0 end as fldBedehkar
,0 as  fldCenterCoId,0 fldCaseId
,case when @Type=1 then (select'('+c.fldCode+')'+c.fldTitle    from acc.tblCoding_Details as c where c.fldid=fldCodingId) else fldName_CodeDetail end fldName_CodeDetail
,isnull(fldCaseTypeId,0) as fldCaseTypeId ,isnull(fldSourceId,0) fldSourceId
, isnull(fldName,N'')fldName
from (
select sum(fldBedehkar) -sum(fldBestankar) as mande
,case when @Type=1 then (select c2.fldId from acc.tblCoding_Details as c2 where c2.fldHeaderCodId=@headerId and c2.fldCode=c.fldCode) else fldCodingId end as fldCodingId
,c.fldTitle,'('+c.fldCode+')'+c.fldTitle as fldName_CodeDetail  
...
outer apply    (select top 1 parent.fldid from     
				...
				where p.fldHeaderCodId=c.fldHeaderCodId and parent.fldLevelId>0  and c.fldCodeId.IsDescendantOf(p.fldCodeId) =1   
				)tempname 
where  (d.fldDocument_HedearId1=h1.fldId or 
			...
			and fldFiscalYearId=@FiscalYearId and h1.fldTypeSanadId in (1,2,3) and fldDocumentNum>0 and fldModuleSaveId=4   and (tempname.fldId not in (7,8) or tempname.fldId  is null)
...
,case when @Type=1 then (select c.fldTitle   from acc.tblCoding_Details as c where c.fldid=fldCodingId) else fldTitle end fldTitle
,N'' as fldDescription
--mande as fldBedehkar, 0  as fldBestankar 
,case when mande>0 then mande else 0 end as fldBedehkar
,case when mande<0 then abs(mande) else 0 end as  fldBestankar 
,0 as  fldCenterCoId,0 fldCaseId
,case when @Type=1 then (select'('+c.fldCode+')'+c.fldTitle    from acc.tblCoding_Details as c where c.fldid=fldCodingId) else fldName_CodeDetail end fldName_CodeDetail
,0 as fldCaseTypeId ,0 fldSourceId, N'' fldName
from (
select sum(fldBedehkar) -sum(fldBestankar) as mande
,case when @Type=1 then (select c2.fldId from acc.tblCoding_Details as c2 where c2.fldHeaderCodId=@headerId and c2.fldCode=n.fldCode) else n.fldid end as fldCodingId
 ,n.fldTitle,'('+n.fldCode+')'+n.fldTitle as fldName_CodeDetail
from acc.tblDocumentRecord_Header as h
inner join acc.tblDocumentRecord_Details as d on d.fldDocument_HedearId=h.fldId
inner join acc.tblDocumentRecord_Header1 as h1 on h1.fldDocument_HedearId=h.fldId
inner join acc.tblFiscalYear as f on f.fldId=h.fldFiscalYearId
...
cross apply    (select top 1 p.fldId,p.fldTitle,p.fldCode,p.fldDaramadCode from     
		acc.tblCoding_Details as p   
		inner join acc.tblTemplateCoding as t on t.fldId=p.fldTempCodingId 
