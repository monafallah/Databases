...
SET ANSI_NULLS ON
GO
CREATE proc [Drd].[spr_RptStatusCeck]
@AzTarikh VARCHAR(10)
,@TaTarikh VARCHAR(10)
,@organId INT
as
begin tran
--declare @AzTarikh VARCHAR(10)='1398/08/11',@TaTarikh VARCHAR(10)='1398/12/11',@organId INT=1
declare @FromDate datetime,@ToDate datetime
set @FromDate=com.ShamsiToMiladi(@AzTarikh)
set @ToDate=com.ShamsiToMiladi(@TaTarikh)+' '+'23:59'
--select @FromDate,@ToDate
SELECT  fldId,fldNameShakhs
  ,  fldTarikh,isnull(fldFather_Sabt,'')fldFather_Sabt,fldMablaghKoli,fldMablaghTakhfif,ISNULL((fldMablaghKoli/*-fldMablaghTakhfif*/),0) AS fldMablaghGHabelPardakht
,fldNameOrgan
 ...
 FROM(SELECT  e.fldId, e.fldDesc,   
            ...
            e.fldTarikh ,
                      ...
                      ,isnull((SELECT isnull(SUM(fldTakhfifMaliyatValue+fldSumAsli+fldTakhfifAvarezValue+fldTakhfifAmuzeshParvareshValue),SUM(fldMaliyatValue+fldSumAsli+fldAvarezValue+fldAmuzeshParvareshValue)) FROM Drd.tblCodhayeDaramadiElamAvarez WHERE fldElamAvarezId=e.fldId),0) fldMablaghKoli,
...
,sum(fldMablaghSanad)over (partition by q.fldElamAvarezId) as fldMablaghTaghsit
 ,isnull(naghd.fldMablaghNaghdi,0) as fldMablaghNaghdi,isnull(fldMablaghNaghdiPardakhtNashode,0) as fldMablaghNaghdiPardakhtNashode
...
outer apply(
	...
	WHERE  e.fldDate BETWEEN @FromDate AND @ToDate
	and e.fldOrganId=@organId
)t
