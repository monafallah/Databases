...
SET ANSI_NULLS ON
GO
CREATE PROC [Pay].[spr_Pay_RptFishHoghoghi_KosuratParam_Bank_Moavaghe_Details]
@fldPersonalId INT,
@NobatPardakht INT,
@Year INT,
@Month INT
...
union all
	...
	FROM         Pay.tblMohasebat INNER JOIN
                      Pay.tblMohasebat_PersonalInfo ON Pay.tblMohasebat.fldId = Pay.tblMohasebat_PersonalInfo.fldMohasebatId
                     where  @dHesabType<>1 and fldPersonalId=@fldPersonalId and fldYear=@Year AND fldMonth=@Month 
...
FROM         [Pay].[tblMohasebat_kosorat/MotalebatParam] INNER JOIN
                      ...
                      Pay.tblMohasebat_PersonalInfo ON Pay.tblMohasebat.fldId = Pay.tblMohasebat_PersonalInfo.fldMohasebatId
                      where   tblMohasebat.fldPersonalId=@fldPersonalId and fldYear=@Year AND fldMonth=@Month 
					  AND fldNobatPardakht=@NobatPardakht  AND fldKosoratId IS NOT NULL  and fldCalcType=@CalcType
...
FROM         Pay.tblMohasebat_KosoratBank INNER JOIN
                      ...
                      Pay.tblMohasebat_PersonalInfo ON Pay.tblMohasebat.fldId = Pay.tblMohasebat_PersonalInfo.fldMohasebatId
                       where  @dHesabType<>1 and tblMohasebat.fldPersonalId=@fldPersonalId and fldYear=@Year AND fldMonth=@Month 
					   AND fldNobatPardakht=@NobatPardakht  and fldCalcType=@CalcType
union all
...
FROM         Pay.tblMoavaghat_Items INNER JOIN
					...
					left join prs.tblHokm_Item as h on h.fldPersonalHokmId=tblMoavaghat.fldHokmId and h.fldItems_EstekhdamId=tblMoavaghat_Items.fldItemEstekhdamId
					  cross apply(select mm.fldKarkard from pay.tblMohasebat as mm where mm.fldPersonalId=m.fldPersonalId and mm.fldYear=tblMoavaghat.fldYear and mm.fldMonth=tblMoavaghat.fldMonth   and mm.fldCalcType=1)mm
                      WHERE  fldHesabTypeItemId=@dHesabType and  m.fldPersonalId=@fldPersonalId and tblMoavaghat.fldYear=@Year 
					  AND tblMoavaghat.fldMonth=@Month     and fldCalcType=1
...
FROM         Pay.tblMoavaghat as o inner join
                      ...
                      Pay.tblMohasebat_PersonalInfo ON m.fldId = Pay.tblMohasebat_PersonalInfo.fldMohasebatId
					   cross apply(select mm.fldKarkard from pay.tblMohasebat as mm where mm.fldPersonalId=m.fldPersonalId and mm.fldYear=o.fldYear and mm.fldMonth=o.fldMonth  and mm.fldCalcType=1 )mm
where  @dHesabType<>1 and fldPersonalId=@fldPersonalId and o.fldYear=@Year AND o.fldMonth=@Month  and fldCalcType=1
...
----------------------------------------------متمم
union all
SELECT  i.fldItemsHoghughiId,i.fldTitle+N'_معوقه متمم ماه قبل',  (o.fldMablagh) AS Expr1,fldPersonalId,m.fldMashmolBime,m.fldMashmolMaliyat,m.fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht,m.fldyear,m.fldmonth,mm.fldKarkard,p.fldHokmId,18 fldtype,3 as fldCalcType
...
union all
SELECT     tblItems_Estekhdam.fldItemsHoghughiId,Com.tblItems_Estekhdam.fldTitle+ a.fldNameMonasebat+'('+t.fldTypeName+')' +N'_معوقه متمم ماه قبل'AS fldTitle,( o.fldMablagh),fldPersonalId,m.fldMashmolBime,m.fldMashmolMaliyat,m.fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht,m.fldyear,m.fldmonth,mm.fldKarkard,p.fldHokmId,19 fldtype,3 as fldCalcType
,case when o.fldMaliyatMashmool= 1 then N'هست' when o.fldMaliyatMashmool= 0 then  N'نیست' else N'نامشخص' end as fldMaliyatMashmool
,case when o.fldBimeMashmool= 1 then N'هست' when o.fldBimeMashmool= 0 then N'نیست' else N'نامشخص' end as fldBimeMashmool
,(isnull(h.fldMablagh,0)) as fldMablaghHokm
FROM         Pay.tblMohasebat_ItemMotamam as o INNER JOIN
...
SELECT     tblItems_Estekhdam.fldItemsHoghughiId,tblItems_Estekhdam.fldTitle +'('+
CASE WHEN fldNesbatShakhs=1 THEN N'فرزند' WHEN fldNesbatShakhs=2 THEN N'همسر' WHEN fldNesbatShakhs=3 THEN N'پدر' WHEN fldNesbatShakhs=4 THEN N'مادر' END
+')' +N'_معوقه متمم ماه قبل'AS fldTitle,( o.fldMablagh),m.fldPersonalId,m.fldMashmolBime,m.fldMashmolMaliyat,m.fldYear as fldYearPardakht,m.fldMonth as fldMonthPardakht,m.fldyear,m.fldmonth,mm.fldKarkard,p.fldHokmId,20 fldtype,3 as fldCalcType
,case when o.fldMaliyatMashmool= 1 then N'هست' when o.fldMaliyatMashmool= 0 then  N'نیست' else N'نامشخص' end as fldMaliyatMashmool
,case when o.fldBimeMashmool= 1 then N'هست' when o.fldBimeMashmool= 0 then N'نیست' else N'نامشخص' end as fldBimeMashmool
,(isnull(h.fldMablagh,0)) as fldMablaghHokm
FROM        Pay.tblMohasebat_ItemMotamam as o  INNER JOIN
