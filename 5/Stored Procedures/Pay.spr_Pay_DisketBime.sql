...
SET ANSI_NULLS ON
GO
CREATE proc [Pay].[spr_Pay_DisketBime] (@sal SMALLINT ,@mah TINYINT,@KargahBime INT,@nobat TINYINT)
as
--declare @sal SMALLINT=1401,@mah TINYINT=1,@KargahBime INT=1,@nobat TINYINT=1
declare @organ int,@Tarikh varchar(10)=cast(@sal as varchar(4))+'/01/01'
...
,fldTarikhTavalod,fldMeliyat,fldJensiyat,NameSodoor,case when fldNoeEstekhdamId<>1 then 0 else fldPayeSanavati end fldPayeSanavati,case when fldNoeEstekhdamId=1 then fldTaahol else 0 end fldTaahol,
CASE WHEN fldBimePersonal=0 THEN cast((round (fldMashmolBime * fldDarsadBimePersonal/100.0,0))as bigint) else cast((ISNULL((SELECT     (fldBimePersonal*Pay.tblKarkardMahane_Detail.fldKarkard)/Pay.tblKarKardeMahane.fldKarkard
FROM         Pay.tblKarKardeMahane INNER JOIN
                      Pay.tblKarkardMahane_Detail ON Pay.tblKarKardeMahane.fldId = Pay.tblKarkardMahane_Detail.fldKarkardMahaneId
                      WHERE Pay.tblKarKardeMahane.fldPersonalId=PersonalId AND Pay.tblKarKardeMahane.fldYear=t.fldYear 
					  AND Pay.tblKarKardeMahane.fldMah=t.fldmonth AND Pay.tblKarKardeMahane.fldNobatePardakht=t.fldNobatPardakht 
					  and fldKargahBimeId=@KargahBime),fldBimePersonal))as bigint) end fldBimePersonal
,CASE WHEN fldBimePersonal=0 THEN cast(fldBimeKarFarma-(round (fldMashmolBime * fldDarsadBimePersonal/100.0,0))as bigint) else cast((ISNULL((SELECT     (fldBimeKarFarma*Pay.tblKarkardMahane_Detail.fldKarkard)/Pay.tblKarKardeMahane.fldKarkard
FROM         Pay.tblKarKardeMahane INNER JOIN
                      Pay.tblKarkardMahane_Detail ON Pay.tblKarKardeMahane.fldId = Pay.tblKarkardMahane_Detail.fldKarkardMahaneId
                      WHERE Pay.tblKarKardeMahane.fldPersonalId=PersonalId AND Pay.tblKarKardeMahane.fldYear=t.fldYear 
					  AND Pay.tblKarKardeMahane.fldMah=t.fldmonth AND Pay.tblKarKardeMahane.fldNobatePardakht=t.fldNobatPardakht 
					  and fldKargahBimeId=@KargahBime),fldBimeKarFarma))as bigint)end fldBimeKarFarma
