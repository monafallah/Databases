...
SET ANSI_NULLS ON
GO
create PROC [Pay].[spr_Pay_DisketBazneshastegi_Mojen](@sal SMALLINT,@mah TINYINT,@Nobat TINYINT,@organId int)
as
--declare @sal SMALLINT=1402,@mah TINYINT=6,@Nobat TINYINT=1,@organId int =1
SELECT 
[fldSh_Personali],fldCodemeli,fldName,fldFamily,fldFatherName,fldSh_Shenasname,fldTarikhTavalod,fldMahalTavalodId
,CASE WHEN fldJensiyat = 0 THEN N'زن' ELSE N'مرد' END AS  fldJensiyat
...
,fldTabaghe,fldTarikhEstekhdam,fldTypeEstekhdamBaz,fldMobile,fldMazad30Sal,fldTarikhEjra,fldYear,fldMonth,fldMashmolBime as fldMashmolBime,fldBimePersonal as fldBimePersonal,fldBimeKarFarma as fldBimeKarFarma
,case when  fldMashmolBimeMoavaghe>0 then fldMashmolBimeMoavaghe else 0  end as  fldMashmolBimeMoavaghe
,case when  fldBimePersonalMoavaghe>0 then fldBimePersonalMoavaghe else 0  end fldBimePersonalMoavaghe--,((((fldMashmolBimeMoavaghe+fldMashmolBime)-fldMogharari)-fldMashmolBime)*0.09)
,case when  fldBimeKarfarmaMoavaghe>0 then fldBimeKarfarmaMoavaghe else 0  end  fldBimeKarfarmaMoavaghe--,(((fldMashmolBimeMoavaghe+fldMashmolBime)-fldMogharari)*0.135)-fldBimeKarFarma
...
ISNULL([riali],'')[riali],[jazb9]=ISNULL([jazb9],0),ISNULL([jazb],0)+ISNULL([ghazai],'') +ISNULL([jazb-tabsare],0)+ISNULL([jazb2],0)+ISNULL([jazb3],0)+ISNULL([jazbTarmim],0)[jazb],
...
FROM(SELECT     Prs.Prs_tblPersonalInfo.fldSh_Personali, tblEmployee.fldCodemeli, tblEmployee.fldName, tblEmployee.fldFamily, fldFatherName, 
					  ...
					  case when Pay.Pay_tblPersonalInfo.fldMazad30Sal=0 then 0 else 0 end as fldMazad30Sal,
                          (SELECT     fldTarikhEjra
                           ...
                           /*ISNULL((SELECT SUM(o.fldMashmolBime) FROM Pay.tblMoavaghat as o WHERE o.fldMohasebatId=Pay.tblMohasebat.fldId GROUP BY fldMohasebatId),0)*/ isnull(Moavaghe.fldMashmolBime,0) fldMashmolBimeMoavaghe
                       , /* ISNULL((SELECT SUM(o.fldBimePersonal) FROM Pay.tblMoavaghat as o  WHERE o.fldMohasebatId=Pay.tblMohasebat.fldId GROUP BY fldMohasebatId),0)*/ISNULL(Moavaghe.fldBimePersonal,0) fldBimePersonalMoavaghe
						,/* ISNULL((SELECT SUM(o.fldBimeKarFarma) FROM Pay.tblMoavaghat as o  WHERE o.fldMohasebatId=Pay.tblMohasebat.fldId GROUP BY fldMohasebatId),0)*/ ISNULL(Moavaghe.fldBimeKarFarma,0) fldBimeKarfarmaMoavaghe
...
FROM         Pay.tblMohasebat INNER JOIN
     ...
     outer apply(SELECT  sum(o.fldMashmolBime)fldMashmolBime,sum(o.fldBimePersonal)fldBimePersonal,sum(o.fldBimeKarFarma)fldBimeKarFarma FROM Pay.tblMoavaghat as o 
