...
SET ANSI_NULLS ON
GO
create proc [Auto].[spr_RptCharkheMessage](@MessageId int)
as
...
DEALLOCATE curCharkhe;
	...
	select  id,sender,null pid,AssId,_SID as C_SID, SenderName,'' TypeAss,'' fldNameStatus,'' fldLetterReadDate,'' fldTarikhEjra,'' fldDesc
					  from @ch 
...
union all 
	select id,sender, pid,AssId,_SID as C_SID ,SenderName,fldType TypeAss,statusname fldNameStatus, statusass.LetterReadDate fldLetterReadDate,statusass.fldTarikhErja fldTarikhEjra, fldDesc
							 from @ch
						  cross apply (SELECT    tblEmployee.fldName COLLATE Latin1_General_CS_AS + ' ' + tblEmployee.fldFamily COLLATE Latin1_General_CS_AS + '(' + tblOrganizationalPostsEjraee.fldTitle + ')'  AS SenderName
					 from tblCommision INNER JOIN
						  com.tblAshkhas ON tblCommision.fldAshkhasID = tblAshkhas.fldID INNER JOIN
