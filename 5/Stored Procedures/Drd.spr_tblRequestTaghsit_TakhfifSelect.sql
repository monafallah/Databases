...
CREATE PROC [Drd].[spr_tblRequestTaghsit_TakhfifSelect] 
...
SELECT     TOP (@h) Drd.tblRequestTaghsit_Takhfif.fldId, Drd.tblRequestTaghsit_Takhfif.fldElamAvarezId, Drd.tblRequestTaghsit_Takhfif.fldRequestType, 
				...
				,CAST((SELECT isnull(SUM(cast(fldtakhfifMaliyatValue+fldtakhfifAvarezValue as bigint)),SUM(cast(fldMaliyatValue+fldAvarezValue as bigint))) FROM Drd.tblCodhayeDaramadiElamAvarez WHERE fldElamAvarezId=Drd.tblRequestTaghsit_Takhfif.fldElamAvarezId AND fldShomareHesabCodeDaramadId IN(SELECT fldid FROM Drd.tblShomareHesabCodeDaramad WHERE fldCodeDaramadId in (SELECT fldId FROM Drd.tblCodhayeDaramd)))AS BIGINT) AS fldMablagh--مبلغ ارزش افزوده
			,CAST((SELECT isnull(SUM(cast(fldTakhfifAmuzeshParvareshValue as bigint)),SUM(cast(fldAmuzeshParvareshValue as bigint))) FROM Drd.tblCodhayeDaramadiElamAvarez WHERE fldElamAvarezId=Drd.tblRequestTaghsit_Takhfif.fldElamAvarezId AND fldShomareHesabCodeDaramadId IN(SELECT fldid FROM Drd.tblShomareHesabCodeDaramad WHERE fldCodeDaramadId in (SELECT fldId FROM Drd.tblCodhayeDaramd)))AS BIGINT) AS fldMablaghAmuzesh
			/*for hidden*/,CAST((SELECT SUM(fldSumAsli) FROM Drd.tblCodhayeDaramadiElamAvarez WHERE fldElamAvarezId=Drd.tblRequestTaghsit_Takhfif.fldElamAvarezId AND fldShomareHesabCodeDaramadId IN(SELECT fldid FROM Drd.tblShomareHesabCodeDaramad WHERE fldCodeDaramadId in (SELECT fldId FROM Drd.tblCodhayeDaramd)))AS BIGINT) AS fldMablaghWithTakhfifKoli
...
SELECT     TOP (@h) Drd.tblRequestTaghsit_Takhfif.fldId, Drd.tblRequestTaghsit_Takhfif.fldElamAvarezId, Drd.tblRequestTaghsit_Takhfif.fldRequestType, 
					...
					,CASE WHEN EXISTS (SELECT * FROM Drd.tblStatusTaghsit_Takhfif WHERE fldRequestId=Drd.tblRequestTaghsit_Takhfif.fldId AND fldTypeMojavez=1) THEN N'توافق'
						WHEN EXISTS(SELECT * FROM Drd.tblStatusTaghsit_Takhfif WHERE fldRequestId=Drd.tblRequestTaghsit_Takhfif.fldId AND fldTypeMojavez=2) THEN N'عدم توافق'
						WHEN NOT EXISTS (SELECT * FROM Drd.tblStatusTaghsit_Takhfif WHERE fldRequestId=Drd.tblRequestTaghsit_Takhfif.fldId) THEN N'بی پاسخ' END AS fldReplyRequestName
					,CAST(0 AS bigint)  AS fldMablaghMashmoolKarmozd,CAST(0 AS bigint)  AS fldMablagh,cast(0 as bigint)fldMablaghAmuzesh
