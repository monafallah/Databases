...
SET ANSI_NULLS ON
GO
CREATE PROC [Pay].[spr_tblSayerPardakhtsInsert] 
...
AS 
	...
	BEGIN TRAN
	declare @fldID int ,@IdMohasebat INT,@flag BIT=0,@PrsPersonalId INT,@fldInsurance INT,@ShomareBime NVARCHAR(50)
	...
	,@fldorganId INT,@fldH_BankFixId INT,@OrganPersonal INT,@ChartOrgan INT
	...
	select @fldID =ISNULL(max(fldId),0)+1 from [Pay].[tblSayerPardakhts] 
	INSERT INTO [Pay].[tblSayerPardakhts] ([fldId], [fldPersonalId], [fldYear], [fldMonth], [fldAmount], [fldTitle], [fldNobatePardakt], [fldMarhalePardakht],  [fldUserId], [fldDate], [fldDesc],fldHasMaliyat,fldMaliyat,fldKhalesPardakhti,fldMostamar)
	SELECT @fldId, @fldPersonalId, @fldYear, @fldMonth, @fldAmount, @fldTitle, @fldNobatePardakt, @fldMarhalePardakht,@fldUserId, GETDATE(), @fldDesc,@fldHasMaliyat,@fldMaliyat,@fldKhalesPardakhti,@fldMostamar
	IF(@@ERROR<>0)
	BEGIN
		ROLLBACK
		SET @flag=1
	...
	IF(@flag=0)
	BEGIN
	SELECT @PrsPersonalId=fldPrs_PersonalInfoId,@fldInsurance=fldInsuranceWorkShopId 
	...
	SET @OrganPersonal=Com.fn_OrganId(@PrsPersonalId)
	...
	SELECT @IdMohasebat,NULL,NULL,NULL,NULL,@fldID,@fldCostCenterId,@fldInsurance,@JobCode,@TypeBime,@AnvaEstekhdam,NULL,NULL,@fldShomareHesabId,@ShomareBime,@Sh_hesabPasAndaz,@Sh_hesabPasKarFarma,
	NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,@EsargariId,@OrganPersonal,@fldUserId,@fldDesc,GETDATE(),@ChartOrgan
	IF (@@ERROR<>0)
		ROLLBACK
