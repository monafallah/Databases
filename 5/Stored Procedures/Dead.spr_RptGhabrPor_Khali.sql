...
--SET QUOTED_IDENTIFIER ON
--GO
CREATE proc [Dead].[spr_RptGhabrPor_Khali](@fieldname nvarchar(50),@Vadiid int,@Gheteid int,@radifid int,@shomareid int,@organid int)
as 
...
if (@fieldname='Khali')
begin
	set @query='select row_number()over ( order by (select 1))id,fldTabaghe fldTedadTabaghat,0 ShomareId,fldGheteId 
		,0 as radifid,'' fldShomare,'' fldNameRadif,fldNameGhete ,v.fldName as fldNamevadiSalam
		 from dead.tblGhabrInfo
inner join dead.tblGhete gg on gg.fldid=fldGheteId
inner join dead.tblVadiSalam v on v.fldid=fldVadiSalamId
where fldStatus=1'
...
if (@fieldname='Por')
begin
	set @query=@query+ 'select row_number()over ( order by (select 1))id,fldTabaghe fldTedadTabaghat,0 ShomareId,fldGheteId 
		,0 as radifid,'' fldShomare,'' fldNameRadif,fldNameGhete ,v.fldName as fldNamevadiSalam
		 from dead.tblGhabrInfo
inner join dead.tblGhete gg on gg.fldid=fldGheteId
inner join dead.tblVadiSalam v on v.fldid=fldVadiSalamId
