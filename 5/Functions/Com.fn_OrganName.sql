...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [Com].[fn_OrganName] (@OrganPostId INT)
RETURNS INT
AS
BEGIN
...
FROM         Com.tblOrganizationalPosts INNER JOIN
                      Com.tblChartOrgan AS tblChartOrgan ON Com.tblOrganizationalPosts.fldChartOrganId = tblChartOrgan.fldId 
                      where Com.tblOrganizationalPosts.fldId=@OrganPostId
