...
SET ANSI_NULLS ON
GO
CREATE function  [Com].[fn_SanavatRasmi](@TarikhEstekhdam varchar(100), @TarikhEjra varchar(100))
returns  nvarchar(100)
begin
	declare @sal int ,@mah int,@roz int,@rozCount int,@CountString  nvarchar(100)=''
	SELECT @rozCount=sum( DATEDIFF(DAY,DBO.Fn_AssembelyShamsiToMiladiDate( @TarikhEstekhdam), DBO.Fn_AssembelyShamsiToMiladiDate(@TarikhEjra)))
