...
CREATE FUNCTION [Drd].[Fn_MablaghTakhfif(Tahlil2)](@FieldName NVARCHAR(50), @Id INT )
RETURNS BIGINT
AS
BEGIN
...
BEGIN
INSERT INTO @r
SELECT ISNULL( (cast(fldTakhfifAsliValue as bigint)*fldTedad),(fldAsliValue*fldTedad))+ISNULL((fldTakhfifAvarezValue),(fldAvarezValue) )+isnull((fldTakhfifMaliyatValue) ,(fldMaliyatValue)) FROM Drd.tblCodhayeDaramadiElamAvarez 
...
INSERT INTO @r
SELECT  CASE WHEN fldTakhfifAsliValue IS NOT NULL THEN((fldAsliValue*fldTedad))-ISNULL((ISNULL(fldTakhfifAsliValue,0)*fldTedad),0) ELSE 0 END  + CASE WHEN fldTakhfifAvarezValue<>0 then((fldAvarezValue)-ISNULL((fldTakhfifAvarezValue),0)) ELSE 0 END + CASE WHEN fldTakhfifMaliyatValue<>0  then
((fldMaliyatValue)-ISNULL((fldTakhfifMaliyatValue),0) ) ELSE 0 END FROM Drd.tblCodhayeDaramadiElamAvarez 
...
BEGIN
INSERT INTO @r
SELECT ISNULL( (fldTakhfifAsliValue*fldTedad),(fldAsliValue*fldTedad))+ISNULL((fldTakhfifAvarezValue),(fldAvarezValue) )+isnull((fldTakhfifMaliyatValue) ,(fldMaliyatValue)) 
FROM            Drd.tblCodhayeDaramadiElamAvarez INNER JOIN
                         Drd.tblShomareHesabCodeDaramad ON Drd.tblCodhayeDaramadiElamAvarez.fldShomareHesabCodeDaramadId = Drd.tblShomareHesabCodeDaramad.fldId INNER JOIN
                         Drd.tblCodhayeDaramd AS tblCodhayeDaramd_1 ON Drd.tblShomareHesabCodeDaramad.fldCodeDaramadId = tblCodhayeDaramd_1.fldId
...
begin
	--INSERT INTO @r
	SELECT @Mablagh= SUM(fldAsliValue*fldTedad)-SUM(fldTakhfifAsliValue*fldTedad)	 FROM  Drd.tblCodhayeDaramadiElamAvarez WHERE fldElamAvarezId=@Id
	--SELECT @Mablagh=sum(mablagh) FROM @r
end
