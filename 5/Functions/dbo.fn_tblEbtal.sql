...
SET ANSI_NULLS ON
GO
CREATE function [dbo].[fn_tblEbtal](@fldId int,@LSN varchar(max))
returns @Resulte table(Id int primary key not null,fldFishId INT,fldRequestTaghsit_TakhfifId int,fldUserID int, fldDesc nvarchar(MAX),fldTarikh NVARCHAR(10),opr int, cdcdate nchar(10), cdctime nchar(8), binLSN varchar(max))
as  
begin
	...
	select @maxlsn=max(__$start_lsn) from (select * from cdc.drd_tblEbtal_CT where (__$start_lsn )<= @LSN) as a
	where fldId=@fldId
	...
	--set @time =cast(sys.fn_cdc_map_lsn_to_time(@maxlsn)as time)
	insert into @Resulte
	Select top(1)  fldId ,fldFishId,fldRequestTaghsit_TakhfifId
	,fldUserID ,isnull( fldDesc,''),dbo.MiladiTOShamsi(flddate)
	,__$operation , dbo.MiladiTOShamsi(@date), cast (@time as char(8))
	, convert(  varchar(max), __$start_lsn,1) from cdc.drd_tblEbtal_CT
