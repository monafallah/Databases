...
SET ANSI_NULLS ON
GO
CREATE function [Pay].[fn_KhalesPardakhtiSayerPardakhtsPardkhati]( @PresonalId int,@Year smallint,@Month tinyint,@NobatPardakht tinyint,@typeHesab tinyint)
returns table
return
		SELECT   sum(fldAmount)  as fldAmount
FROM         Pay.tblSayerPardakhts INNER JOIN
       ...
       WHERE fldYear=@Year AND fldMonth=@Month AND fldNobatePardakt=@NobatPardakht 
					  AND   Pay.tblSayerPardakhts.fldPersonalId=@PresonalId 
